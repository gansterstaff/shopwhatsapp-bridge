...
const ProductDetail = () => {
  ...
  const categoryInSpanish = getCategoryInSpanish(product.category);

  return (
    <>
      <Helmet>
        <title>{`${product.name} | Compra Online | Mejor Precio`}</title>
        <meta name="description" content={`${product.description.substring(0, 150)}... ✓ Envío rápido ✓ Garantía de calidad ✓ Mejores precios`} />
        <meta name="keywords" content={`${product.name}, ${categoryInSpanish}, comprar ${product.name}, ${product.category}, mejor precio, oferta, descuento, online`} />
        <meta property="og:title" content={`${product.name} | Tienda Online`} />
          ...
          <div className="space-y-6">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <Badge variant="outline">{categoryInSpanish}</Badge>
                {product.stock > 0 ? (
